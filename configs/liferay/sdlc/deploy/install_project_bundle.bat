...
:find_bundle_zip
	...
	for /f "delims=" %%a in ('dir /s /b CUS*.zip') do (
	 set "bundle_path=%%a"
     set bundle_name=%%~nxa
    )
...
:move_new_bundle
	...
	echo a|xcopy "%BUNDLE_FOLDER%\temp" "%LIFERAY_FOLDER%" /H /E /Q /C /F /I /S /Y /D
