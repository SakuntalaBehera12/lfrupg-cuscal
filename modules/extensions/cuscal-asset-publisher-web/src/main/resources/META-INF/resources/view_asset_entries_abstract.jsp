...
<%-- CUSCAL: START CUSTOMIZATION --%>
<ul class="custom-css-title-list list-group show-quick-actions-on-hover">
	<%-- CUSCAL: END CUSTOMIZATION --%>
	...
	%>
		...
		<li class="list-group-item list-group-item-flex <%= ((previewClassNameId == assetEntry.getClassNameId()) && (previewClassPK == assetEntry.getClassPK())) ? "active" : StringPool.BLANK %>" <%= AUIUtil.buildData(fragmentsEditorData) %>>
			...
			>
				...
				<%
				String summary = assetRenderer.getSummary(liferayPortletRequest, liferayPortletResponse);

				if (summary != null) {
					summary = StringUtil.shorten(summary, assetPublisherDisplayContext.getAbstractLength(), "");
				}
				else {
					summary = "";
				}
				...
				<c:if test="<%= displayDate != null %>">
					<%-- CUSCAL: START CUSTOMIZATION --%>
					<p class="asset-date list-group-subtitle ml-2 text-truncate">
						<%-- CUSCAL: END CUSTOMIZATION --%>
				...
				<%-- CUSCAL: START CUSTOMIZATION --%>
				<p class="asset-summary list-group-subtitle pb-2 pt-1 text-truncate">
					<%= summary %>
				</p>
				<%-- CUSCAL: END CUSTOMIZATION --%>
			...
			<%-- CUSCAL: START CUSTOMIZATION --%>
			<%-- No asset_actions on abstracts display
			<clay:content-col>
