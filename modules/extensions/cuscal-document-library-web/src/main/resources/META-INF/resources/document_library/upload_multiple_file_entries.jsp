						...
						>
							...
							<aui:script use="aui-base,aui-loading-mask-deprecated,node-load">
								...
								window['<portlet:namespace />updateMultipleFiles'] = function () {
									...
									if(A.one('#<portlet:namespace />approvalNotes').val() === '') {

										if(commonFileMetadataContainer.getHTML().toString().search('Please enter approval notes') == -1) {
											commonFileMetadataContainer.prepend('<div class="alert alert-dismissible alert-danger" role="alert"> <button aria-label="Close" class="close" data-dismiss="liferay-alert" type="button"> <span id="bahj____"><svg aria-hidden="true" class="lexicon-icon lexicon-icon-times" focusable="false"><use href="/o/admin-theme/images/clay/icons.svg#times"></use></svg></span> <span class="sr-only">Close</span> </button> <span class="alert-indicator"> <svg aria-hidden="true" class="lexicon-icon lexicon-icon-exclamation-full"> <use xlink:href="/o/admin-theme/images/clay/icons.svg#exclamation-full"></use> </svg> </span> <strong class="lead">Error:</strong>Please enter approval notes </div>');
										}
