...
import com.liferay.portal.security.ldap.internal.CuscalPortalLDAPContext;
...
@Component(
	...
	service = UserExporter.class
)
public class CuscalLDAPUserExporterImpl implements UserExporter {
	...
	public void exportUser(
		...
		if (orgs.isEmpty()) {
			if (_log.isErrorEnabled()) {
				_log.error(
					"exportToLDAP Contact : LDAP contact can't have empty organization");
			}
		...
		if (Validator.isNull(passwd)) {
			if (_log.isErrorEnabled()) {
				_log.error(
					"exportToLDAP  ERROR  : LDAP contact can't have empty password");
			}
		...
		try {
			...
			SafeLdapName name = SafeLdapNameFactory.from(userBinding);

			if (_log.isDebugEnabled()) {
				_log.debug(
					"ExportToLDAP : Name on  contact Modifing  is: " + name);
			}

			final Organization org0 = user.getOrganizations(
			).get(
				0
			);
			final String cuscalBSB = CuscalLDAPSyncUtil.getOrgCustomValue(
				user, org0, "cuscalBSB");

			if (_log.isDebugEnabled()) {
				_log.debug(
					"cuscalBSB for " + org0.getName() + " is 8 : " + cuscalBSB);
			}

			if (cuscalBSB != null) {
				modifications.addItem(
					"cuscalBSB",
					CuscalLDAPSyncUtil.getOrgCustomValue(
						user, orgs.get(0), "cuscalBSB"));
			}

			final String cuscalCUOrgId = CuscalLDAPSyncUtil.getOrgCustomValue(
				user, org0, "cuscalCUOrgId");

			if (_log.isDebugEnabled()) {
				_log.debug(
					"PortalLDAPExporterImpl: cucalCUOrgId for" +
						org0.getName() + " is 9 : " + cuscalCUOrgId);
			}

			if (cuscalCUOrgId != null) {
				modifications.addItem(
					"cuscalCUOrgId",
					CuscalLDAPSyncUtil.getOrgCustomValue(
						user, orgs.get(0), "cuscalCUOrgId"));
			}

			safeLdapContext.modifyAttributes(name, modificationItems);
		}
		finally {
			if (safeLdapContext != null) {
	...
	public void exportUser(
		...
		if (!_ldapSettings.isExportEnabled(companyId) ||
			...
			if (_log.isInfoEnabled()) {
				_log.info(
					"LDAP Export is Not enabled, we not exporting to LDAP ... ");
			}
	...
	public void exportUser(
		...
		if (orgs.isEmpty()) {
			if (_log.isErrorEnabled()) {
				_log.error("LDAP User can't have empty organization");
			}
		...
		if (passwd == null) {
			if (_log.isErrorEnabled()) {
				_log.error("LDAP User can't have empty password");
			}
		...
		try {
			...
			if (_log.isDebugEnabled()) {
				_log.debug(
					"Name on Modify submit :" + userSafeLdapName.toString());
			}

			if (_log.isDebugEnabled()) {
				_log.debug("Adding cuscalBSB to modified items ");
			}
			...
			if (_log.isDebugEnabled()) {
				_log.debug(
					"ExportToLDAP cuscalBSB for " + org0.getName() + " is : " +
						cuscalBSB);
			}
			...
			if (_log.isDebugEnabled()) {
				_log.debug(
					"ExportToLDAP cucalCUOrgId for" + org0.getName() +
						" is : " + cuscalCUOrgId);
			}
			...
			if (_log.isDebugEnabled()) {
				_log.debug(
					" >>>>>>>>>>>>>> ExportToLDAP-User Modifing LDAP Done >>>>>>>>>>>>");
			}
	...
	protected Binding addGroup(
		...
		if (_log.isDebugEnabled()) {
			_log.debug(
				"addGroup : Name before Bidning to LDAP is :  " +
					userGroupSafeLdapName.toString());
		}

		ldapContext.bind(
			userGroupSafeLdapName, new CuscalPortalLDAPContext(attributes));

		if (_log.isDebugEnabled()) {
			_log.debug(
				" >>>>>>>>>>>>>>> AddGroup :Binding to LDAP Done >>>>>>>>>>>>");
		}
	...
	protected Binding addUser(
		...
		if (_log.isDebugEnabled()) {
			_log.debug("addUser:putting attributes for cuscalBSB");
		}

		if (user.getOrganizationIds().length > 0) {
			final Organization org0 = user.getOrganizations(
			).get(
				0
			);
			...
			if (cuscalBSB != null) {
				attributes.put("cuscalBSB", cuscalBSB);
			}

			if (cuscalCUOrgId != null) {
				attributes.put("cuscalCUOrgId", cuscalCUOrgId);
			}
		}

		ldapContext.bind(
			userSafeLdapName, new CuscalPortalLDAPContext(attributes));

		if (_log.isDebugEnabled()) {
			_log.debug(
				" >>>>>>>>>>AddUser : Binding to LDAP Done >>>>>>>>>>>>");
		}
	...
	private static final Log _log = LogFactoryUtil.getLog(
		CuscalLDAPUserExporterImpl.class);
	...
	@Reference(
		target = "(component.name=com.liferay.portal.security.ldap.internal.exportimport.CuscalPortalToLDAPConverter)"
	)
