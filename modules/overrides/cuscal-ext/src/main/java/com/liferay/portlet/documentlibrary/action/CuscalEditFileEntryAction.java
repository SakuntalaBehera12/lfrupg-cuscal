...
import com.liferay.portlet.common.action.CuscalActionUtil;
...
/**
 * Based on original Liferay Source Code:
 * https://github.com/liferay/liferay-portal-ee/blob/7.3.x/modules/apps/document-library/document-library-web/src/main/java/com/liferay/document/library/web/internal/portlet/action/EditFileEntryMVCActionCommand.java
 *
 * @author Maurice Sepe
 */
@Component(
...
public class CuscalEditFileEntryAction extends BaseMVCActionCommand {

	public static final String TEMP_FOLDER_NAME =
		CuscalEditFileEntryAction.class.getName();
	...
	protected void doProcessAction(
		...
		try {
			...
			if (uploadException != null) {
				...
				/* -- CUSCALSOW4-153 START -- */
				final String body = ParamUtil.getString(
					(PortletRequest)actionRequest,
					CuscalActionUtil.APPROVAL_NOTES);

				if ((body == null) || body.isEmpty()) {
					SessionErrors.add(
						(PortletRequest)actionRequest, "invalidApprovalNotes");
				...
				/* -- CUSCALSOW4-282 START -- */
				final String body = ParamUtil.getString(
					actionRequest, CuscalActionUtil.APPROVAL_NOTES);

				if ((body == null) || body.isEmpty()) {
					SessionErrors.add(actionRequest, "invalidApprovalNotes");
	...
	private void _addMultipleFileEntries(
		...
		try {
			...
			FileEntry fileEntry = _dlAppService.addFileEntry(
				null, repositoryId, folderId, uniqueFileName, mimeType,
				uniqueFileName, StringPool.BLANK, description, changeLog,
			...
			/* -- CUSCALSOW4-282 START -- */
			CuscalActionUtil.addCommentToWorkflow(
	...
	private String _getAddMultipleFileEntriesErrorMessage(
		...
		else if (exception instanceof FileSizeException fileSizeException) {
			errorMessage = themeDisplay.translate(
	...
	private String[] _getAllowedFileExtensions(
		...
		_settings = FallbackKeysSettingsUtil.getSettings(
			new CompanyServiceSettingsLocator(
				...
				"com.liferay.document.library.web.internal.settings.DLPortletInstanceSettings"));

		_typedSettings = new TypedSettings(_settings);

		Set<String> extensions = new HashSet<>();

		String[] mimeTypes = _typedSettings.getValues(
			"mimeTypes",
		...
		try (InputStream inputStream = uploadPortletRequest.getFileAsStream(
			...
			if (cmd.equals(Constants.ADD) ||
				...
				if (portletName.equals(DLPortletKeys.MEDIA_GALLERY_DISPLAY)) {
					...
					int count = Arrays.binarySearch(mimeTypes, contentType);
			...
			if (cmd.equals(Constants.ADD)) {
				...
				fileEntry = _dlAppService.addFileEntry(
					...
					title, StringPool.BLANK, description, changeLog,
					inputStream, size, null, null, null, serviceContext);
			}
				...
				String uniqueFileName = DLUtil.getUniqueFileName(
					themeDisplay.getScopeGroupId(), folderId, title, false);
				...
				fileEntry = _dlAppService.updateFileEntryAndCheckIn(
					...
					dlVersionNumberIncrease, inputStream, size, null, null,
					null, serviceContext);
			}
			else {
	...
	private static final Log _log = LogFactoryUtil.getLog(
		CuscalEditFileEntryAction.class);
