 ...
 * https://github.com/liferay/liferay-portal-ee/blob/7.3.x/modules/apps/users-admin/users-admin-web/src/main/java/com/liferay/users/admin/web/internal/portlet/action/EditUserMVCActionCommand.java
 *
...
public class CuscalEditUserMVCActionCommand extends BaseMVCActionCommand {
	...
	protected User addUser(ActionRequest actionRequest) throws Exception {
		...
		// When the user is added, they will automatically be added to the currently
		// selected organisation in the control panel
		...
		long creatorUserId = ParamUtil.getLong(actionRequest, P_U_I_D);

		User user = userLocalService.addUser(
			creatorUserId, themeDisplay.getCompanyId(), true, null, null,
			autoScreenName, screenName, emailAddress,
			LocaleUtil.fromLanguageId(languageId), firstName, middleName,
			lastName, prefixId, suffixId, male, birthdayMonth, birthdayDay,
			birthdayYear, jobTitle, UserConstants.TYPE_REGULAR, null,
			organizationIds, null, null, sendEmail, serviceContext);
	...
	protected void deleteUsers(ActionRequest actionRequest) throws Exception {
		...
		for (long deleteUserId : deleteUserIds) {
			if (cmd.equals(Constants.DEACTIVATE) ||
				...
				userLocalService.deleteUser(deleteUserId);
			}
		}
	}
	...
	protected long getListTypeId(
		...
		ListType listType = _listTypeLocalService.addListType(
			CompanyThreadLocal.getCompanyId(), parameterValue, type);
	...
	protected Object[] updateUser(
		...
		User user = portal.getSelectedUser(actionRequest, false);
	...
	private static final String P_U_I_D = "p_u_i_d";
